...
- **Habits** — daily, weekdays, weekends, picked days or monthly habits, each with a 365-day contribution heatmap, a colour, emoji and optional target streak. One tap logs today. Habits can also **count**: allow several logs a day with a per-day target (eight glasses of water), and the heatmap ramps in four steps as the day fills. Each card shows current streak, lifetime total, best streak, completion rate and progress through the current period. Habits can be paused (keeping their history) or deleted.
