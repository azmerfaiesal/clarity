...
- **Quick capture** — the Create button beside search fans Task, Routine, and Note actions straight upward on the web and up-left on iPhone. Each action opens its full composer from the selected icon. Desktop task views also keep the inline "Add a task" form (or press `N` in Inbox, Today, Upcoming, Favorites, or a category), while compact layouts use the task modal. Expand it for description, due date, priority, category, tags, and reminders. Only the title is required. The current view supplies useful defaults: Today uses today, Upcoming uses tomorrow, a category preselects itself, and Favorites stars the task so it remains visible there. Cancel, tapping the backdrop, or `Escape` dismisses the composer.
